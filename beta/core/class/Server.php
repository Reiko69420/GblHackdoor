...
class Server
{
	...
	// Ajoute un serveur
	public function AddServer($name, $ip, $users, $key, $pw, $map, $gm, $rcon, $players, $additonnals)
	{
		$GLOBALS['DB']->Insert("server_list", ["server_players" => $players, "rcon" => $rcon, "gm" => $gm, "map" => $map, "pw" => $pw, "userid" => $key, "server_name" => $name, "server_ip" => $ip, "server_users" => $users, "last_update" => time(), "payload_call" => -1, "payload_arg" => "", "additonnals" => $additonnals]);
        Logs::AddLogs("<p class='text-primary'>[".date('d/m/Y à H:i:s', time())."]&nbsp;<i class='fa fa-plus'></i>&nbsp;Un nouveau serveur est connecté : ".$name."</p>");
	}

	// Mets à jour un serveur
	public function UpdateServer($server_id, $name, $ip, $users, $pw, $map, $gm, $rcon, $players, $additonnals)
	{
