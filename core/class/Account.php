...
class Account
{
    ...
    public function Auth($username, $password)
    {
        if (Account::isUsernameExist($username))
        {
            ...
            {
                if($GLOBALS['DB']->GetContent('users', ['username' => $username])[0]["verif"] == 0){
                    return false; die("Compte en cours de vérification");
                }else{
                $_SESSION['id'] = $user["id"];
    ...
    public function SetCredits($id, $credits)
    {
        ...
        $GLOBALS['DB']->Update('users', ['id' => $_SESSION['id']], ['credit' => $credits]);
    }
    ...
    public function CreateUserVerif($username, $password, $confirm_password, $discord)
    {
        $ip = $_SERVER['REMOTE_ADDR'];
        if ($password != $confirm_password)
        {
        ...
        $GLOBALS['DB']->Insert("users", ["username" => $username, "password" => $password_protection, "banned" => "", "ip" => "", "useragent" => "", "lastconnected" => time(), "rank" => "FREE", "description" => "Nouveau utilisateur !!", "color" => "1c1c1c", "color_second" => "", "color_trois" => "", "pdp" => "http://i11.servimg.com/u/f11/15/00/95/05/duck_t10.png", "flags" => "", "discord" => "", "verif" => 0, "discordname" => $discord]);

        Logs::AddLogs("<p class='text-primary'>[".date('d/m/Y à H:i:s', time())."]&nbsp;<i class='fa fa-plus'></i>&nbsp;Le nouvelle utilisateur ".htmlentities($username)." à été créer par cette ip : ".$ip." et est en attente de verification</p>");

        httpPost("https://discordapp.com/api/webhooks/647123736721358865/c1EG1wQHX2A5-b_gWe9Wddfih_1IAst4K-aNTScznbb6ud4Qzc8lIKrkGS5W7Pv26j0b", ["content" => "Un Nouveau compte vien d'étre créer par cette ip : ```".$ip."``` Username : ```".$username."``` ``` Discord Name : ```".$discord."```"]);
